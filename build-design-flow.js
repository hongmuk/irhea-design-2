...
const PRINCIPLES = [
  ...
  { num:'S2.1.7', title:'즐겨찾기 vs 매뉴얼 큐 분리 (v1.1)',  detail:'즐겨찾기 = 자주 쓰는 슬롯 (영구). 매뉴얼 큐 = 다음 N잔 대기열 (head→tail, 추출 후 pop). 둘은 메인에서 별도 영역.' },
...
const SCREENS = [
  ...
  ['S2.1', '/main', '메인 모니터링 (5구 도넛 + 매뉴얼 큐)', 'daily', '연결 성공 / 모든 화면 복귀', '게스트(조회만)~', '⭐ 핵심 화면 / 즐겨찾기 5슬롯(5/24) + 매뉴얼 큐 5×6(v1.1 S2.1.7) 둘 다 노출'],
...
const TRANSITIONS = [
  ...
  // 매뉴얼 큐 (v1.1 S2.1.7 — 5/24 보류 → 추후 추가 결정)
  ['S2.1 매뉴얼 큐',    '셀 클릭 (빈 또는 채움)',        'recipe-picker → 큐에 추가/변경', 'v1.1 S2.1.7'],
  ['S2.1 매뉴얼 큐',    '셀 클릭 → [비우기]',            '큐 슬롯 비움 + Toast',   'v1.1 S2.1.7'],
  ['S2.1 매뉴얼 큐',    '[전체 비우기] 클릭',            'P-IRREVERSIBLE → 모든 큐 비움', 'v1.1 S2.1.7'],
  ['S2.1 매뉴얼 큐',    '큐 head (왼쪽) 셀',             '다음 추출 시 자동 선택 (백엔드 협의)', 'v1.1 S2.1.7'],
  ['S2.1',              '추출 완료 (물리 버튼 release)', '큐 head pop → 다음 셀로 shift', 'v1.1 — 백엔드 구현 시']
];
...
const CONFLICTS_PENDING = [
  ...
  ['v1.1 S2.1.7',   '매뉴얼 큐 그리드 5×6',          '즐겨찾기와 분리된 별도 영역',    '5/24 언급 없음 (둘 다 적용 결정)', '✅ 적용 — stations 하단 별도 패널 (셀 클릭=picker, 전체 비우기, head=빨강)', 'OK',     ''],
...
const TASKS = [
  ...
  ['T01', 'barista', 'daily', '커피 추출 모니터링 (실제 시작은 물리 버튼)', '바리스타 (게스트~)', '하루 N회', '3~5분', '장비 앞 물리 버튼으로 시작 → 태블릿 모니터링',
    ...
    [['사이드바 [메인]','/main 5구 카드 + 매뉴얼 큐'],
     ['레시피 박스/슬롯 클릭 (선택)','confirm "OO → XX로 변경?"'],
     ['장비 앞으로','머신 추출구 앞'],
     ['물리 버튼 누름','WHP → status: extracting (큐 head 적용)'],
     ['진행률·단계 N/M 확인','단계별 progress'],
     ['완료 → 30초 점멸','서빙 후 자동 idle, 큐 head pop']]],
  ...
  ['T04b', 'barista', 'daily', '매뉴얼 큐로 다음 추출 줄세우기 (v1.1 S2.1.7)', '바리스타', '바쁜 시간', '20초', '5×6 큐에 다음 추출 N잔 미리 입력 — 즐겨찾기와 분리',
    '큐 head(왼쪽 빨강) = 다음 물리 버튼 누를 때 적용될 레시피. 추출 완료 시 자동 pop & shift.',
    [['메인 하단 매뉴얼 큐 영역','5 추출구 × 6 셀'],
     ['빈 셀 클릭','recipe-picker (allowClear)'],
     ['레시피 선택','localStorage override 저장 + 셀 채움'],
     ['반복 또는 [전체 비우기]','head=빨강, 나머지=골드']]],
